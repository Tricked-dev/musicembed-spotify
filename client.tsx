...
for (; ;) {
    try {
        ...
        const text = await Bun.$`playerctl metadata --format "{{artist}}%{{title}}%{{mpris:artUrl}}%{{mpris:length}}%{{position}}"`.text().then(r => r.trim().split("%"))
        ...
        if (!errored)
            console.log("Cooking failed error occured ", e)
        errored = true
    }
}